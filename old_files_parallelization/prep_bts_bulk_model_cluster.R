...
if(standardise_data){
  ...
  obs_data$unif_0[obs_data$unif_0 < 0] = 0 #this will ignore the NA 
...
generate_bts = function(bts_rng){
  ...
  for(b in bts_rng){
    ...
    process_station_data <- function(station_data, quantile_models) {
      ...
      # Extract scale and shape parameters for each model
      scle_0 <- station_data$scale_0
      shpe_0 <- station_data$shape_0
      scle_1 <- station_data$scale_1
      shpe_1 <- station_data$shape_1
      scle_2 <- station_data$scale_2
      shpe_2 <- station_data$shape_2
      scle_3 <- station_data$scale_3
      shpe_3 <- station_data$shape_3
      
      threshold <- station_data$threshold_9
      my_lambda <- station_data$thresh_exceedance_9
      
      station_data$maxtp_0 <- NA
      station_data$maxtp_1 <- NA
      station_data$maxtp_2 <- NA
      station_data$maxtp_3 <- NA
      
      for(i in seq(nrow(station_data))) {
        
        # --- model 0
        if(!is.na(my_lambda[i]) && !is.na(station_data$unif_0[i]) && station_data$unif_0[i] > (1 - my_lambda[i])) {
          station_data$maxtp_0[i] <- evd::qgpd(
            p = (1 + (station_data$unif_0[i] - 1) / my_lambda[i]),
            loc = threshold[i], scale = scle_0[i], shape = shpe_0[i]
          )
        } else if (!is.na(station_data$unif_0[i])) {
          station_data$maxtp_0[i] <- apply_tau_to_temp(quantile_models, station_data$year[i], station_data$stn[i], station_data$unif_0[i])
        }
        
        # Repeat similar logic for models 1, 2, and 3
        # Model 1
        if(!is.na(my_lambda[i]) && !is.na(station_data$unif_1[i]) && station_data$unif_1[i] > (1 - my_lambda[i])) {
          station_data$maxtp_1[i] <- evd::qgpd(
            p = (1 + (station_data$unif_1[i] - 1) / my_lambda[i]),
            loc = threshold[i], scale = scle_1[i], shape = shpe_1[i]
          )
        } else if (!is.na(station_data$unif_1[i])) {
          station_data$maxtp_1[i] <- apply_tau_to_temp(quantile_models, station_data$year[i], station_data$stn[i], station_data$unif_1[i])
        }
        
        # Model 2
        if(!is.na(my_lambda[i]) && !is.na(station_data$unif_2[i]) && station_data$unif_2[i] > (1 - my_lambda[i])) {
          station_data$maxtp_2[i] <- evd::qgpd(
            p = (1 + (station_data$unif_2[i] - 1) / my_lambda[i]),
            loc = threshold[i], scale = scle_2[i], shape = shpe_2[i]
          )
        } else if (!is.na(station_data$unif_2[i])) {
          station_data$maxtp_2[i] <- apply_tau_to_temp(quantile_models, station_data$year[i], station_data$stn[i], station_data$unif_2[i])
        }
        
        # Model 3
        if(!is.na(my_lambda[i]) && !is.na(station_data$unif_3[i]) && station_data$unif_3[i] > (1 - my_lambda[i])) {
          station_data$maxtp_3[i] <- evd::qgpd(
            p = (1 + (station_data$unif_3[i] - 1) / my_lambda[i]),
            loc = threshold[i], scale = scle_3[i], shape = shpe_3[i]
          )
        } else if (!is.na(station_data$unif_3[i])) {
          station_data$maxtp_3[i] <- apply_tau_to_temp(quantile_models, station_data$year[i], station_data$stn[i], station_data$unif_3[i])
        }
      }
    ...
    # Set up parallel processing using all cores except one
    cl <- makeCluster(detectCores() - 1)
    clusterExport(cl, varlist = c("apply_tau_to_temp"))  # Export necessary variables
    clusterEvalQ(cl, library(tidyverse) )  # Load required libraries on each worker
    
    unique_stations <- unique(expanded_data$stn)
    
    # Run the process in parallel
    results <- parLapply(cl, unique_stations, function(stn){
      chunk_data <- expanded_data %>% filter(stn == !!stn)
      process_station_data(chunk_data, quantile_models)
    })
    
    # Combine the results back into a single data frame
    combined_expanded_data <- bind_rows(results)
    
    # Stop the cluster
    stopCluster(cl)
    
    combined_expanded_data %>%
    plyr::rbind.fill() %>%
    as_tibble() %>%
...
i = 1
batch_size = 20
generate_bts(seq(i,(i+batch_size-1))) #not  inside a job, scared of memory issues 
