#computing the CV metrics for the fitted parameters of the GPD
    ...
    for(s in seq(num_spatial_folds)){
      for(t in week_chunks){
        ...
        write.table(paste0("model.0", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_9,  pred_0$scale, pred_0$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/plain_spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
  ...
  if(cv_method == "12fold"){
    ...
    for(i in seq(num_random_folds)){
      ...
      write.table(paste0("model.0", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_9,  pred_0$scale, pred_0$shape[1])),
                  file = paste0('output/cv_res/plain_twelve_fold_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                  sep = ",", append = TRUE, quote = FALSE,
                  col.names = FALSE, row.names = FALSE)
...
spatio_temporal_metrics = read.csv("output/cv_res/plain_spatio_temporal_cv_test_mod_n9.csv", header=FALSE)
...
twelvefold_metrics = read.csv("output/cv_res/plain_twelve_fold_cv_test_mod_n9.csv", header=FALSE)
