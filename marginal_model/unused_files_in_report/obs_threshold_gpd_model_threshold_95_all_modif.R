...
obs_data = obs_data %>%
  group_by(stn)%>%
  mutate(obs_quant_95 = quantile(maxtp, 0.95))%>%
  ungroup %>%
  mutate(obs_threshold_95 = clim_thresh_value_95/2 + obs_quant_95/2)

obs_data %>%
  saveRDS(paste0("Data/processed/obs_threshold_obs_data_95_for_bulk_model_num_quantiles_",num_quantiles,".csv")) #adding the 95 to avoid mistakes
#messing everything up

#obs_data = readRDS(paste0("Data/processed/obs_data_95_for_bulk_model_num_quantiles_",num_quantiles,".csv"))

obs_data = obs_data %>%
  rename(threshold_95 = obs_threshold_95)
...
lambda_thresh_ex %>%
  write_csv(paste0("Data/processed/obs_threshold_glob_anomaly_95_thresh_exceedance_lambda_num_quantiles_",num_quantiles,".csv"))
...
threshold_95_df = readRDS(paste0("Data/processed/obs_threshold_obs_data_95_for_bulk_model_num_quantiles_",num_quantiles,".csv"))%>%
  rename(threshold_95 = obs_threshold_95)%>%
  dplyr::select(threshold_95, stn)%>%
  unique()

#lambda associated with observed data, vary the quantile model
lambda_df = vroom::vroom("Data/processed/obs_threshold_glob_anomaly_95_thresh_exceedance_lambda_num_quantiles_30.csv") 
...
obs_data = obs_data %>% 
  ...
  left_join(threshold_95_df, by="stn")
...
if(fit_true_models){
  ...
  fit_mod_0(extreme_dat_true$excess, extreme_dat_true$scale_95, c(0.4, 0.1, -0.1))  %>%
    ...
    write_csv("output/gpd_model_fits/obs_threshold_model_0_true_95.csv")
  
  fit_mod_1(extreme_dat_true$excess, extreme_dat_true$scale_95,
            extreme_dat_true$glob_anom, c(0.2, 0.6, 0.2, -0.1))  %>%
    matrix() %>% t() %>% as.data.frame() %>%
    write_csv("output/gpd_model_fits/obs_threshold_model_1_true_95.csv")
  ...
  fit_mod_2(extreme_dat_true$excess, extreme_dat_true$scale_95,
    ...
    matrix() %>% t() %>% as.data.frame() %>%
    write_csv("output/gpd_model_fits/obs_threshold_model_2_true_95.csv")
  
  fit_mod_3(extreme_dat_true$excess, extreme_dat_true$scale_95,
            extreme_dat_true$altitude, c(0.7, 0.01, -0.03, -0.1))  %>%
    matrix() %>% t() %>% as.data.frame() %>%
    write_csv("output/gpd_model_fits/obs_threshold_model_3_true_95.csv")
...
list_ip = list(c(0.1, 0.1, 0.5, -0.1), c(-1, 0.1, 0.5, -0.1), c(0.1, 1, -0.5, -0.1))

for (ip in list_ip){
   ip = c(0.2, 0.6, 0.2, -0.1)
  
   opt_param = fit_mod_1(extreme_dat_true$excess, extreme_dat_true$scale_95,
                         extreme_dat_true$glob_anom, ip)
   print(opt_param)
   print(ngll_1(opt_param))
