...
bulk_and_tail_MAG = bulk_and_tail  %>% filter(stn == "MAG") %>% unique()
bulk_and_tail_LSN = bulk_and_tail  %>% filter(stn == "LSN") %>% unique()
bulk_and_tail_RIE = bulk_and_tail  %>% filter(stn == "RIE") %>% unique()
bulk_and_tail_JUN = bulk_and_tail  %>% filter(stn == "JUN") %>% unique()
bulk_and_tail_ALT = bulk_and_tail  %>% filter(stn == "ALT") %>% unique()

standardised_qq_MAG = standardised_qq  %>% filter(stn == "MAG")%>% unique()
standardised_qq_LSN = standardised_qq  %>% filter(stn == "LSN") %>% unique()
standardised_qq_RIE = standardised_qq  %>% filter(stn == "RIE") %>% unique()
standardised_qq_JUN = standardised_qq  %>% filter(stn == "JUN") %>% unique()
standardised_qq_ALT = standardised_qq  %>% filter(stn == "ALT") %>% unique()

plot_title = "Altdorf, 437m"

plt = gridExtra::grid.arrange(bulk_and_tail_ALT %>%
                                drop_na() %>%
                                ggplot()+
                              ...
                              standardised_qq_ALT %>%
                                ggplot()+ #below don't know how to modify such that it works 
                                #geom_ribbon(data = tibble(qnt = standardised_qq_MAG$rank, (lower_ci), (upper_ci)), aes(x = qnt, ymin = sort(lower_ci), ymax = sort(upper_ci)), alpha = 0.3)+
                              ...
                              bottom = textGrob("Ideal quantiles", rot = 0, vjust = 0, 
                                                gp = gpar(col = "black", fontsize = 12)),
                              top = textGrob(plot_title, gp = gpar(fontsize = 14, fontface = "bold"))
)
