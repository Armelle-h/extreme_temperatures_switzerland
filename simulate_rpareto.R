...
variogram_model <- function(h){
  nu = 0.2 # used to be 0.2 but was done for km, I'm using meters
  res = rep(NA, length(h))
  res[h == 0] = 0
...
fit = read_csv(paste0("output/rpareto_model_fits/plain_true_rpareto_fits_model_",marg_mod, ".csv"),
               col_names = F) %>% 
...
for (index in c(1, 26, 51, 76)) {
  job::job (
    {
      file_name = paste0("output/simulations/simulations_on_obs_grid/true/",marg_mod,"_run_")
