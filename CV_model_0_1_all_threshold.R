  ...
  if(cv_method == "spatial-temporal"){
    ...
    for(s in seq(num_spatial_folds)){
      for(t in week_chunks){
        ...
        this_fit_mod_0 = fit_mod_0(train$excess, train$scale )
        this_fit_mod_1 = fit_mod_1(train$excess, train$scale , train$glob_anom)
        this_fit_mod_2 = fit_mod_2(train$excess, train$scale , train$glob_anom, train$altitude)
        this_fit_mod_3 = fit_mod_3(train$excess, train$scale , train$altitude)
        
        # calculate scale parameter on climate grid
        pred_0 = my_predict_0(this_fit_mod_0, test$scale)
        pred_1 = my_predict_1(this_fit_mod_1, test$scale, test$glob_anom)
        pred_2 = my_predict_2(this_fit_mod_2, test$scale, test$glob_anom, test$altitude)
        pred_3 = my_predict_3(this_fit_mod_3, test$scale, test$altitude)
        
        write.table(paste0("model.0", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold,  pred_0$scale, pred_0$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.1", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold,  pred_1$scale, pred_1$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.2", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold,  pred_2$scale, pred_2$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.3", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold,  pred_3$scale, pred_3$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
      }
