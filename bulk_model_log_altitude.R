...
if(fit_clim_quants){
  ...
  file.remove(paste0("Data/processed/log_altitude_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"))
  ...
  for(q in seq_along(quantiles_to_estimate_bulk)){
    ...
    tibble(tau = zeta,
      ...
      write_csv(paste0("Data/processed/log_altitude_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"), append = T)
  }
}
...
quant_reg_model_pars = read_csv(paste0("Data/processed/log_altitude_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"),
    ...
    for(q in seq_along(quantiles_to_estimate_bulk)){
      ...
      res = rbind(res,
                  tibble(quantile =  qpars$tau,
                         ...
                         quant_value = qpars$beta_0 + qpars$beta_1*clim_vals[q] + (qpars$beta_2) *log(temporal_covariates$altitude)))
    }
...
obs_smoothed_quantiles %>% saveRDS(paste0("output/log_altitude_quant_models_num_quantiles_",num_quantiles,".csv"))
...
lambda_thresh_ex %>%
  write_csv(paste0("Data/processed/altitude_thresh_exceedance_lambda_num_quantiles_",num_quantiles,".csv"))
  ...
  for(q in seq_along(quantiles_to_estimate_bulk)){
    ...
    res = rbind(res,
                tibble(quantile =  qpars$tau,
                       ...
                       quant_value = qpars$beta_0 + qpars$beta_1*clim_vals[q] + (qpars$beta_2)*(temporal_covariates$altitude)   ))
  }
