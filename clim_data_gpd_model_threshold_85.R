...
clim_data_extreme_85_list = list()

for (i in seq_along(files)){
  
  clim_data = fread(files[[i]])
  
  #keeping only temperatures above the associated threshold 0.85 quantile
  sing_clim_data_extreme_85 = clim_data %>%
    group_by(id) %>%
    mutate(threshold = quantile(maxtp, 0.85),
           excess = maxtp - threshold) %>%
    filter(excess > 0) %>%
    ungroup()
  
  clim_data_extreme_85_list[[i]] = sing_clim_data_extreme_85
...
clim_data_extreme_85 =  do.call(rbind, clim_data_extreme_85_list)

rm(clim_data_extreme_85_list)
...
clusterExport(cl, c("sub_clim_data_extreme_85", "estimate_scale_fixed_shape", "ngll"))
...
for (potential_shape in potential_shape_values_climate) {
  ...
  process_id <- function(i, potential_shape) {
    this_clim_extm_irel <- sub_clim_data_extreme_85 %>%
      filter(id == i) %>%
      pull(excess)
  ...
  results <- parLapply(cl, unique(sub_clim_data_extreme_85$id), process_id, potential_shape)
...
print(optimal_shape)
saveRDS(optimal_shape, file = "temp_optimal_shape_85.rds")
min_loglik_sum = min(loglik_sum)
print(min_loglik_sum)
saveRDS(min_loglik_sum, file = "temp_optimal_loglikelihood_85.rds")

#DO THE SPLINE THING TO GET THE MAX AND THEN RUN THE CODE BELOW

potential_shape_values_climate = readRDS("shape_candiates_85.rds")
loglik_sum = readRDS("associated_loglikelihood_85.rds")
...
saveRDS(optimal_shape, "optimal_shape_85.rds")
saveRDS(estimated_optimal_loglik, "optimal_loglikelihood_85.rds")
...
id_clim_data_extreme_85 = clim_data_extreme_85 %>%
  dplyr::select(id, excess)%>%
  filter(id %% 5 == 0)  #for comparison purpose with the other loglikelihoods computed 

process_id = function(i, optimal_shape){
  this_clim_extm_irel_85 <- id_clim_data_extreme_85 %>% filter(id == i) %>% pull(excess)
  # Estimate scale parameter with the optimal shape parameter
  model_fit_85 <- estimate_scale_fixed_shape(this_clim_extm_irel_85, optimal_shape)
  return(list(par = model_fit_85$par, loglik= model_fit_85$value))
}
...
indices <- unique(id_clim_data_extreme_85$id)

n <- length(unique(id_clim_data_extreme_85$id))
...
}, import=c("job_process_id", "chunks", "optimal_shape", "process_id", "id_clim_data_extreme_85", "estimate_scale_fixed_shape", "ngll")
...
saveRDS(loglik_optimal_shape, "optimal_loglikelihood_5_index_85.rds") #loglikelihood computed on one out of 5 indexes
...
scales_85 <- c(result_1$scales_, result_2$scales_, result_3$scales_, result_4$scales_, result_5$scales_)


#end of new version

saveRDS(loglik_optimal_shape, "loglik_optimal_shape_85.rds") #loglikelihood computed for all indexes 

saveRDS(scales_85, "scales_85.rds")

scales_85 = readRDS("scales_85.rds")

# --- save estimates on climate grid

#keeping only location where the temperature is above the associated threshold 0.85 quantile
clim_data_extreme_85 %>%
  dplyr::select(id) %>%
  unique() %>%
  mutate(scale_85 = scales_85) %>%
  write_csv("Data/Climate_data/clim_scale_grid_85_gpd_model.csv")
...
obs_sites = obs_sites %>%
  left_join(clim_data_extreme_85 %>%
              dplyr::select(id) %>%
              unique() %>%
              mutate(scale_85 = scales_85) %>%
              dplyr::select(id, scale_85), by = 'id')

obs_data %>%
  left_join(obs_sites) %>% 
  write_csv("Data/Observed_data/obs_data_85_gpd_model.csv")