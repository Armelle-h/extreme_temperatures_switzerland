...
obs_data = read.csv("Data/processed/obs_threshold_1971_2022_JJA_obs_data_bulk_model.csv") %>%
  select(-id, -clim_thresh_value_9, -obs_quant_9)

num_quantiles = 30 

lambda_thresh_ex = read.csv(paste0("Data/processed/obs_threshold_glob_anomaly_thresh_exceedance_lambda_num_quantiles_",num_quantiles,".csv"))

emp_proba = obs_data %>%
  group_by(stn, year)%>%
  summarize(emp_proba = mean(maxtp > obs_threshold), .groups = 'drop')

emp_proba = emp_proba %>%
  inner_join(lambda_thresh_ex, by = c("stn", "year"))

emp_proba = emp_proba %>%
  mutate(diff = emp_proba-thresh_exceedance_9)%>%
  mutate(abs_diff = abs(emp_proba-thresh_exceedance_9))



#not estimating using outliers

num_quantiles = 30
obs_data = readRDS(paste0("Data/processed/obs_data_for_bulk_model_num_quantiles_",num_quantiles,".csv"))

threshold_9_df = read.csv("Data/processed/obs_threshold_1971_2022_JJA_obs_data_bulk_model.csv") %>%
  rename(threshold_9 = obs_threshold)%>%
  select(stn, year, threshold_9)%>%
  unique()
...
obs_data = obs_data %>% 
  ...
  left_join(threshold_9_df, by = c("stn", "year"))
...
obs_data_filtered = obs_data %>%
  group_by(stn)%>%
  filter(maxtp < quantile(maxtp, 0.999) & maxtp>quantile(maxtp, 0.001))

#inside the if loop depends only on climate data, not the observed data
if(fit_clim_quants){
  
  file.remove(paste0("Data/processed/no_outlier_glob_anomaly_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"))
  ...
  for(q in seq_along(quantiles_to_estimate_bulk)){
    ...
    obs_data_for_quant_reg = rlang::duplicate(obs_data_filtered, shallow = FALSE)
    ...
    tibble(tau = zeta,
      ...
      write_csv(paste0("Data/processed/no_outlier_glob_anomaly_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"), append = T)
  }
}
...
quant_reg_model_pars = read_csv(paste0("Data/processed/no_outlier_glob_anomaly_quantile_model_fit_pars_num_quantiles_",num_quantiles,".csv"),
...
lambda_thresh_ex = obs_data %>%
  ...
  group_map(~{
    ...
    thresh_exceedance_9 = obs_smoothed_quantiles%>%
      filter(stn == .x$stn[1]) %>%
      pull(temp_to_tau) %>%
      sapply(function(x) sapply(.x$threshold_9.y[1], x))
...
emp_proba = obs_data %>%
  group_by(stn, year)%>%
  summarize(emp_proba = mean(maxtp > threshold_9.y), .groups = 'drop')
