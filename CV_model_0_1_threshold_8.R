...
threshold_8_df = vroom::vroom("Data/processed/1971_2022_JJA_obs_data_bulk_model.csv")%>%
...
run_cv = function(cv_method, thresh_qnt, obs_data, spatial_folds, get_metrics, num_spatial_folds = "NA", week_chunks="NA"){
  ...
  if(cv_method == "spatial-temporal"){
    ...
    for(s in seq(num_spatial_folds)){
      for(t in week_chunks){
        ...
        this_fit_mod_0 = fit_mod_0(train$excess, train$scale_8)
        this_fit_mod_1 = fit_mod_1(train$excess, train$scale_8, train$glob_anom)
        this_fit_mod_2 = fit_mod_2(train$excess, train$scale_8, train$glob_anom, train$altitude)
        this_fit_mod_3 = fit_mod_3(train$excess, train$scale_8, train$altitude)
        
        # calculate scale parameter on climate grid
        pred_0 = my_predict_0(this_fit_mod_0, test$scale_8)
        pred_1 = my_predict_1(this_fit_mod_1, test$scale_8, test$glob_anom)
        pred_2 = my_predict_2(this_fit_mod_2, test$scale_8, test$glob_anom, test$altitude)
        pred_3 = my_predict_3(this_fit_mod_3, test$scale_8, test$altitude)
        
        write.table(paste0("model.0", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_0$scale, pred_0$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.1", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_1$scale, pred_1$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.2", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_2$scale, pred_2$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
        
        write.table(paste0("model.3", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_3$scale, pred_3$shape[1]),",",mean(t)),
                    file = paste0('output/cv_res/spatio_temporal_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                    sep = ",", append = TRUE, quote = FALSE,
                    col.names = FALSE, row.names = FALSE)
      }
  ...
  if(cv_method == "10fold"){
    ...
    for(i in seq(num_random_folds)){
      ...
      write.table(paste0("model.0", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_0$scale, pred_0$shape[1])),
                  file = paste0('output/cv_res/ten_fold_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                  sep = ",", append = TRUE, quote = FALSE,
                  col.names = FALSE, row.names = FALSE)
      
      write.table(paste0("model.1", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_1$scale, pred_1$shape[1])),
                  file = paste0('output/cv_res/ten_fold_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                  sep = ",", append = TRUE, quote = FALSE,
                  col.names = FALSE, row.names = FALSE)
      
      write.table(paste0("model.2", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_2$scale, pred_2$shape[1])),
                  file = paste0('output/cv_res/ten_fold_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                  sep = ",", append = TRUE, quote = FALSE,
                  col.names = FALSE, row.names = FALSE)
      
      write.table(paste0("model.3", ",", get_metrics(test$maxtp, test$excess, test$quant, test$threshold_8,  pred_3$scale, pred_3$shape[1])),
                  file = paste0('output/cv_res/ten_fold_cv_test_mod_n', str_remove(thresh_qnt, "0."), '.csv'),
                  sep = ",", append = TRUE, quote = FALSE,
                  col.names = FALSE, row.names = FALSE)
...
job::job({run_cv('10fold', 0.8, obs_data, spatial_folds, get_metrics)}, import = c("obs_data", 'run_cv', "spatial_folds", "get_metrics"))
job::job({run_cv('spatial-temporal', 0.8, obs_data, spatial_folds, get_metrics, num_spatial_folds, week_chunks)}, import = c("obs_data", 'run_cv', "spatial_folds",  "get_metrics", "num_spatial_folds", "week_chunks"))


spatio_temporal_metrics = read.csv("output/cv_res/spatio_temporal_cv_test_mod_n8.csv", header=FALSE)
...
tenfold_metrics = read.csv("output/cv_res/ten_fold_cv_test_mod_n8.csv", header=FALSE)
